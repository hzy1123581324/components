    ...
    <view class="switch" :class="[value == true ? 'switch-on' : '', disabled ? 'switch-disabled' : '']" @tap="onClick"
     :style="[switchStyle]">
        <view class="switch-node node-class">
            <loading :show="loading" class="switch-loading" :size="size * 0.6" :color="loadingColor" />
        </view>
    </view>
</template>
...
<script>
     ...
     * @example <switch v-model="checked" active-color="red" inactive-color="#eee"></switch>
     */
    ...
    export default {
        ...
        computed: {
            switchStyle() {
                let style = {};
                style.fontSize = uni.upx2px(this.size) + 'px';
...
<style lang="scss" scoped>
    .switch {
        // --size: 30upx;
        // font-size: var(--size);
        // --switch-width: calc(var(--size) * 2);
        // --switch-height: var(--size);
        position: relative;
