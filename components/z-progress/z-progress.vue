<template>
	<view class="progress" :style="{
		...
		<view :class="[
			type ? `type-${type}-bg` : '',
			striped ? 'striped' : '',
			striped && stripedActive ? 'striped-active' : ''
		]" class="active" :style="[progressStyle]">{{showPercent ? percent + '%' : ''}}
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <line-progress :percent="70" :show-percent="true"></line-progress>
	 */
	export default {
		name: "line-progress",
		props: {
