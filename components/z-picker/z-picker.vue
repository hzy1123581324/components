<template>
  ...
  <view class="w_100">
    <view class="picker-inset" @click="handlBefore">
        ...
        <view class="picker-head flex-between-x flex-center-y">
          ...
          <slot name="head">
            <view class="cancel" @click="currentColumn--" v-if="isPrevious">
              <slot name="previous">{{ "上一步" }}</slot>
            </view>
            <view class="cancel" @click="closeHandler" v-else>
              <slot name="cancel">{{ "取消" }}</slot>
            </view>
            <view class="picker-title">
              <!-- <slot name="title">{{ "标题" }}</slot> -->
            </view>
            <view class="confirm" v-if="!isNext" @click="confirmHandler">
              <slot name="confirm">{{ "确定" }}</slot>
            </view>
            <view class="confirm" v-else @click="currentColumn++">
              <slot name="next">{{ "下一步" }}</slot>
            </view>
          </slot>
        </view>
        <picker-view
          ...
          :style="`--picker-col:${col};--picker-column:${column}`"
          @change="bindChange"
        >
          <picker-view-column
            :class="[
              'picker-column',
              parentIndex == 0 && rangeData.length > column && 'picker-column-move',
            ]"
            v-for="(itemParents, parentIndex) in rangeData"
            :key="parentIndex"
            :style="`min-width:${100 / column}%;`"
...
<script>
import { ref, reactive, computed, nextTick, isReactive, watch } from "vue";
import { isObject } from "../../utils/test.js";
...
emits:[
  ...
  setup(props,{emit}){
    ...
    let isNext = computed(() => {
      return (
        currentColumn.value < rangeData.value.length - 1 &&
        rangeData.value.length > props.column
      );
    });
    ...
    function exportData() {
      ...
      for (let i = 0; i < rangeData.value.length; i++) {
        ...
        arrt.push({
          ...
          data: {
            ...rangeData.value[i][currentIndex[i] || 0],
          },
        });
      }
      return arrt;
