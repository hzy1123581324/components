  ...
  <view class="w_100">
    ...
    <z-popup v-model="show" type="bottom" @close="closeHandler">
      <view class="picker-box" :style="`--current-column:${currentColumn}`">
        ...
        <picker-view class="picker-view" indicator-class="picker-indicator" v-model:value="currentIndex"
          :style="`--picker-col:${col};--picker-column:${column}`" @change="bindChange">
          <picker-view-column :class="[
              'picker-column',
              parentIndex == 0 && rangeData.length > column && 'picker-column-move',
            ]" v-for="(itemParents, parentIndex) in rangeData" :key="parentIndex"
            :style="`min-width:${100 / column}%;`" @click="currentColumn = parentIndex">
