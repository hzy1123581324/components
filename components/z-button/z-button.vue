<template>
  <view :class="['button-box f_nowrap',plain&&'plain',shape,disable&&'disable']" @click="buttonClick($event)">
    <z-loading class="mar-rg-20" mode="flower" v-if="isLoading" ></z-loading>
    <slot></slot>
    <!-- 特殊功能，仅button能触发 -->
    <button type="default" v-if="false"></button>
    <!-- 水波效果 -->
    <view v-if="ripple" :class="['wave-ripple', waveActive ? 'wave-active' : '']" :style="rippleStyle"></view>
  </view>
</template>
